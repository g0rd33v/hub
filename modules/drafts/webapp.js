...
// Telegram Mini App dashboard: state API, bot management, analytics, broadcast, wizard
...
const buildPAPState = (project) => {
  ...
  const langs = Object.entries(
    subs.reduce((acc, s) => { const l = s.language_code || 'unknown'; acc[l] = (acc[l]||0)+1; return acc; }, {})
  ...
  return {
    ...
    github: { repo: project.github_repo || null, autosync: project.github_autosync || false },
...
const buildAAPState = (project, aap) => {
  ...
  const sn   = _ctx.config.serverNumber;
  return {
    tier: 'aap', name: project.name, aap_name: aap.name || aap.id, branch: aap.branch,
    live_url: `${base}/${project.name}/`,
    aap_url:  `${base}/signin/pass_${sn}_agent_${aap.token.replace(/^aap_/, '')}`,
    api_base: `${base}/drafts`, aap_token: aap.token,
  };
};
...
const readAnalyticsSummary = (projectName) => {
  const fp = path.join(_ctx.config.dataDir || '/var/lib/hub', 'projects', projectName, 'analytics-summary.json');
...
const buildAnalytics = (project) => {
  const subs = project.bot?.subscribers || [];
  const byLang = {}; const byCoun = {};
  subs.forEach(s => {
    const l = s.language_code || 'unknown'; byLang[l] = (byLang[l]||0)+1;
    if (s.country) byCoun[s.country] = (byCoun[s.country]||0)+1;
  });
  const base = {
    events_total: 0, users_total: subs.length, users_active_7d: 0, users_active_30d: 0,
    subscribed: subs.length, unsubscribed: 0, last_event_at: null,
    by_language: byLang, by_country: byCoun,
    by_hour_utc: Array(24).fill(0), by_dow_utc: Array(7).fill(0),
    by_command: {}, analytics_enabled: project.bot?.analytics_enabled ?? true,
  };
...
// ── Wizard: parse prompt → project plan ─────────────────────────────────────────

const WIZARD_SYSTEM = `You are a project planner for Hub — a platform that hosts Telegram bots, websites, and APIs.
Your job: parse a user prompt and return a JSON project plan. ONLY return JSON, no markdown fences.

Required fields:
- name: (string) short slug, lowercase, hyphens only, max 32 chars
- description: (string) one sentence, max 80 chars
- type: (string) one of: bot | site | api | mixed
- stack: (string) brief tech summary, e.g. "Telegram bot, OpenRouter LLM, polling"
- description_long: (string) 2–3 sentences explaining what this project does
- needs: (array) list of credentials user will need to connect. Only include relevant ones:
    bot_token, openrouter_key, openai_key, anthropic_key, webhook_url, domain

Examples:
Prompt: "daily crypto price bot for my channel"
{
  "name": "crypto-bot",
  "description": "Daily crypto price updates via Telegram bot",
  "type": "bot",
  "stack": "Telegram bot, CoinGecko API, polling",
  "description_long": "A Telegram bot that fetches crypto prices from CoinGecko and broadcasts them daily to subscribers. Supports /subscribe and /unsubscribe commands.",
  "needs": ["bot_token"]
}

Prompt: "AI assistant that uses OpenRouter to answer questions"
{
  "name": "ai-assistant",
  "description": "AI chatbot powered by OpenRouter",
  "type": "bot",
  "stack": "Telegram bot, OpenRouter API, polling",
  "description_long": "A Telegram bot that routes user messages to any LLM via OpenRouter. Supports multi-turn conversations with context window.",
  "needs": ["bot_token", "openrouter_key"]
}

Be concise. Return valid JSON only.`;

const wizardGenerate = async (prompt) => {
  // Try OpenRouter first, fallback to direct Anthropic
  const orKey  = (() => { try { return fs.readFileSync('/etc/hub/OPENROUTER_API_KEY','utf8').trim(); } catch { return ''; } })();
  const antKey = (() => { try { return fs.readFileSync('/etc/hub/ANTHROPIC_API_KEY','utf8').trim(); } catch { return ''; } })();

  if (!orKey && !antKey) {
    // No LLM key — return a generic plan parsed from the prompt
  ...
  if (orKey) {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'system', content: WIZARD_SYSTEM }, ...messages],
        max_tokens: 512,
      }),
    });
  ...
  if (antKey) {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-haiku-4-5-20251001', system: WIZARD_SYSTEM,
...
const fallbackPlan = (prompt) => {
  // Minimal heuristic plan when no LLM key is configured
  const lower = prompt.toLowerCase();
  const hasTg  = lower.includes('bot') || lower.includes('telegram');
  const hasAI  = lower.includes('ai') || lower.includes('gpt') || lower.includes('llm') || lower.includes('openrouter');
  const hasSite = lower.includes('site') || lower.includes('web') || lower.includes('landing');

  const type  = hasTg ? (hasSite ? 'mixed' : 'bot') : hasSite ? 'site' : 'api';
  const needs = [];
  if (hasTg)  needs.push('bot_token');
  if (hasAI)  needs.push('openrouter_key');

  // Generate slug from first few words
  const slug = prompt.toLowerCase().replace(/[^a-z0-9\s]/g,'').trim()
    .split(/\s+/).slice(0,3).join('-').slice(0,32) || 'my-project';

  return {
    name:             slug,
    description:      prompt.slice(0,78).trim(),
    type,
    stack:            hasTg ? 'Telegram bot' + (hasAI ? ', OpenRouter LLM' : '') : hasSite ? 'Static site' : 'API service',
    description_long: `A Hub project: ${prompt.slice(0,140)}`,
    needs,
  };
};
...
export function mountWebAppRoutes(app, ctx) {
  ...
  app.get('/hub/api/analytics/download', (req, res) => {
    const p = authPAP(req, ctx);
    if (!p) return res.status(401).json({ ok: false, error: 'unauthorized' });
    const fp = path.join(ctx.config.dataDir || '/var/lib/hub', 'projects', p.name, 'analytics.jsonl');
  ...
  // Wizard: generate project plan from prompt
  app.post('/hub/api/wizard/generate', async (req, res) => {
    const prompt = String(req.body?.prompt || '').trim();
    if (!prompt) return res.status(400).json({ ok: false, error: 'prompt_required' });
    try {
      const plan = await wizardGenerate(prompt);
      return res.json({ ok: true, plan });
    } catch (e) {
      ctx.logger.warn('[wizard] generate failed:', e.message);
      // Fallback to heuristic plan
      try {
        return res.json({ ok: true, plan: fallbackPlan(prompt) });
      } catch (e2) {
  ...
  app.post('/hub/api/bot/sync', async (req, res) => {
    ...
    try {
      ...
      if (botJson.commands?.length) {
        const cmds = botJson.commands.filter(c => c.command && c.description)
  ...
  app.post('/hub/api/broadcast', async (req, res) => {
    ...
    for (const sub of subs) {
      ...
      try {
        ...
        const data = await r.json();
        if (data.ok) sent++;
        else if (data.error_code === 403 || data.error_code === 400) { dead.push(chatId); skipped++; }
        else skipped++;
      } catch { skipped++; }
    }
    if (dead.length) {
  ...
  // HTML shell (also handles ?mode=wizard)
