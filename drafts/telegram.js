...
function statePath() { return path.join(DRAFTS_DIR, '.telegram.json'); }

function loadState() {
  try {
    if (fs.existsSync(statePath())) {
      telegramState = JSON.parse(fs.readFileSync(statePath(), 'utf8'));
    }
  } catch (e) {
    console.error('[telegram] state load failed:', e.message);
  }
  if (!telegramState || telegramState.version !== STATE_VERSION) {
    telegramState = {
  ...
  if (!('notify_sap_on_drafts_event' in telegramState.settings)) {
    telegramState.settings.notify_sap_on_drafts_event = true;
    persistState();
  }
}
...
function persistState() {
  try {
    ...
    safeWriteJSON(statePath(), telegramState);
  } catch (e) {
    console.error('[telegram] state save failed:', e.message);
  }
}
...
function loadTBP() {
  try {
    ...
    console.error('[telegram] TBP load failed:', e.message);
    TBP = null;
  }
}
...
function persistTBP() {
  try {
    ...
    console.error('[telegram] TBP save failed:', e.message);
  }
}
...
function getUser(tg_user_id) {
  return telegramState.users[String(tg_user_id)] || null;
}

function ensureUser(tgUser) {
  const id = String(tgUser.id);
  if (!telegramState.users[id]) {
    telegramState.users[id] = {
    ...
    telegramState.users[id].tg_username = tgUser.username || telegramState.users[id].tg_username;
    telegramState.users[id].first_name = tgUser.first_name || telegramState.users[id].first_name;
    telegramState.users[id].is_premium = !!tgUser.is_premium;
  }
  return telegramState.users[id];
}

function bindToken(tg_user_id, recognized) {
  const user = telegramState.users[String(tg_user_id)];
  if (!user) return;
...
function findUsersBoundToTier(tier) {
  const out = [];
  for (const [id, u] of Object.entries(telegramState.users)) {
...
async function sendNewPrompt(chatId) {
  const html =
    ...
    'paste the API token from <a href="https://t.me/BotFather">@BotFather</a>\n\n' +
    '<i>looks like</i> <code>1234567890:AA...</code>\n\n' +
    'no bot yet? open @BotFather → /newbot → copy the token.';
...
function setPending(tgUserId, stage) {
  pendingCreate.set(tgUserId, { stage: stage || 'awaiting_token', expires_at: Date.now() + PENDING_TTL_MS });
}
...
// Hub /new flow: project from bot token
async function actuallyCreateFromBotToken(chatId, user, botToken) {
  ...
  try {
    ...
    if (!data.ok || !data.result) {
      return await tgSend(chatId, '<b>token rejected</b>\n\nTelegram says: ' + esc(data.description || 'invalid'));
    }
    botInfo = data.result;
  } catch (e) {
    return await tgSend(chatId, 'could not reach Telegram: ' + esc(e.message));
  }
  if (!botInfo.username) {
    return await tgSend(chatId, 'this token has no username. odd. try another.');
  }
  // 2. Derive project name from bot username (lowercase, valid chars)
  const rawName = botInfo.username.toLowerCase();
  const v = validateProjectName(rawName);
  if (!v.ok) {
    return await tgSend(chatId, 'bot username <code>@' + esc(botInfo.username) + '</code> is not a valid project name. needs a–z, 0–9, -, _ (2–40 chars).');
  }
  if (findProjectByName(v.name)) {
    return await tgSend(chatId,
      'project <code>' + esc(v.name) + '</code> already exists. each bot is one project. open /my to see yours.'
    );
  }
  // 3. Create project
  ...
  let projectRes;
  try {
    projectRes = await serverHelpers.createProject({
      name: v.name,
      description: 'Telegram bot @' + botInfo.username + ' (' + owner + ')',
    });
  } catch (e) {
    let msg = 'project create failed: ' + esc(e.message);
    if (e.message === 'reserved_name') msg = 'name <code>' + esc(v.name) + '</code> is reserved. pick another bot.';
    return await tgSend(chatId, msg);
  }
  bindToken(user.tg_user_id, { tier: 'pap', token: projectRes.pap_token, project: { name: projectRes.project } });
  // 4. Install bot on project
  try {
    const project = findProjectByName(v.name);
    if (project && projectBotsApi.installBot) {
      await projectBotsApi.installBot(project, botToken, { webhook_url: null });
    }
  } catch (e) {
    console.error('[telegram] installBot failed:', e.message);
    // Project was created, but bot binding failed — let user know
    await tgSend(chatId,
      'project created, but bot install failed: ' + esc(e.message) + '\n\nyou can install manually from the dashboard.'
    );
  }
  // 5. Success message
  const html =
    '<b>done</b>\n\n' +
    '<code>' + esc(projectRes.project) + '</code>\n' +
    '→ <a href="' + esc(projectRes.live_url) + '">' + esc(projectRes.live_url) + '</a>\n' +
    'bot: <a href="https://t.me/' + esc(botInfo.username) + '">@' + esc(botInfo.username) + '</a>\n\n' +
    'open the dashboard to manage everything.';
  await tgSend(chatId, html, {
    reply_markup: { inline_keyboard: dashboardKeyboardForBinding({ tier: 'pap', token: projectRes.pap_token, project_name: projectRes.project }) },
  });
  notifySAPOwners('new project: <code>' + esc(projectRes.project) + '</code> · @' + esc(botInfo.username) + ' by ' + esc(owner));
}
...
async function handleMessage(msg) {
  ...
  if (pending) {
    if (pending.stage === 'awaiting_token') {
      // Hub flow: text is bot token
      const tokenCandidate = text.trim();
      if (!/^\d+:[A-Za-z0-9_-]{30,}$/.test(tokenCandidate)) {
        return await tgSend(chatId,
          'that does not look like a bot token. paste the full string from @BotFather, or /cancel.\n\n' +
          '<i>looks like</i> <code>1234567890:AA...</code>'
        );
      }
      clearPending(msg.from.id);
      await tgSend(chatId, 'checking with Telegram...');
      return await actuallyCreateFromBotToken(chatId, user, tokenCandidate);
    }
    // Legacy name flow (kept for callback rename action)
...
async function handleNewCommand(chatId, user, _requestedName) {
  // Hub flow: always ask for bot token. Ignore inline arguments.
  setPending(user.tg_user_id, 'awaiting_token');
...
async function pollLoop() {
  ...
  while (polling && TBP && TBP.token) {
    try {
      ...
      if (e.code === 401) {
        console.error('[telegram] bot token rejected (401). Stopping polling.');
        polling = false;
        break;
      }
...
async function configureBotProfile() {
  ...
  try {
    ...
    console.log('[telegram] bot profile configured (v0.8)');
  } catch (e) {
    console.error('[telegram] configureBotProfile error:', e.message);
  }
}
...
function notifySAPOwners(html) {
  ...
  for (const u of subs) {
    tgSend(u.tg_user_id, html).catch(e => console.error('[telegram] notify SAP failed:', e.message));
  }
}
...
function notifySAPOnDraftsEvent(html) {
  if (!TBP) return;
  if (!telegramState || !telegramState.settings.notify_sap_on_drafts_event) return;
  notifySAPOwners(html);
}

function notifyPAPOwners(projectName, html) {
  if (!TBP) return;
  const subs = Object.values(telegramState.users).filter(u =>
    u.notif_subscribed && u.bindings.some(b => b.tier === 'pap' && b.project_name === projectName)
  );
  for (const u of subs) {
    tgSend(u.tg_user_id, html).catch(e => console.error('[telegram] notify PAP failed:', e.message));
  }
}

function onNewProject(project) {
  if (!telegramState.settings.notify_sap_on_new_project) return;
...
function onAAPMerged(project, aap, versionN) {
  if (!telegramState.settings.notify_pap_on_aap_merge) return;
...
function onMainCommit(project, commit, versionN) {
  if (!telegramState.settings.notify_pap_on_main_commit) return;
...
function mountRoutes(app) {
  ...
  app.put('/drafts/tbp/settings', requireSAP, (req, res) => {
    ...
    for (const k of allowed) if (k in req.body) telegramState.settings[k] = !!req.body[k];
    persistState();
    res.json({ ok: true, settings: telegramState.settings });
  });
  ...
  app.get('/telepath/api/state/:tier/:token?', initDataAuth, (req, res) => {
    ...
    if (tier === 'sap') {
      ...
      return res.json({ ok: true, tier: 'sap', server_number: SERVER_NUMBER, public_base: PUBLIC_BASE, projects, settings: telegramState.settings });
    }
    if (tier === 'pap') {
      const u = getUser(req.tgUser.id);
...
export function initTelegram(opts) {
  ...
  if (TBP && TBP.token) {
    ...
    console.log('[telegram] no TBP installed — bot inactive. Install via PUT /drafts/tbp');
  }
}

export function mountTelegramRoutes(app) { mountRoutes(app); }
...
export function getTelegramStatus() {
  return {
    ...
    polling,
    users_count: telegramState ? Object.keys(telegramState.users).length : 0,
  };
}
